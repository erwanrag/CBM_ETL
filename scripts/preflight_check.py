...
import pyodbc
import pandas as pd
from utils.connections import get_progress_connection, get_sqlserver_connection
...
def check_sqlserver_connection():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM config.ETL_Tables")
        count = cursor.fetchone()[0]
        conn.close()
...
def check_table_config(table_name):
    ...
    try:
        ...
        df_table = pd.read_sql("""
            SELECT * FROM config.ETL_Tables WHERE TableName = ?
